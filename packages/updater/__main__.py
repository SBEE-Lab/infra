#!/usr/bin/env python3
"""
Update third-party packages in ./packages directory.
...
def get_current_version(pkg: Package) -> str | None:
    ...
    default_nix = pkg.path / "default.nix"
...
def run_nix_update(pkg: Package, flake_root: Path, dry_run: bool = False) -> bool:
    ...
    result = subprocess.run(cmd, check=False, cwd=flake_root, capture_output=True, text=True)
...
def update_package(pkg: Package, flake_root: Path, dry_run: bool = False) -> UpdateResult:
...
def create_pr_for_package(pkg: Package, flake_root: Path, dry_run: bool = False) -> bool:
...
def _create_pr_in_worktree(pkg: Package, flake_root: Path, branch_name: str) -> bool:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        try:
            return _run_update_and_create_pr(pkg, flake_root, worktree_path, branch_name)
        finally:
...
def _run_update_and_create_pr(
    ...
    worktree_pkg = Package(
        name=pkg.name,
        method=pkg.method,
        path=worktree_path / "packages" / pkg.name,
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Update third-party packages in ./packages")
    parser.add_argument(
        "--dry-run",
        action="store_true",
