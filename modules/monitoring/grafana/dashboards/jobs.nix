...
# Batch/sync/backup health. Current source is psi systemd status snapshots in Loki.
{ datasources }:
let
...
in
{
  ...
  panels = [
    ...
    (statPanel {
      ...
      title = "borg failures";
      x = 16;
      expr = ''sum(count_over_time({host="psi", log_type="systemd_status", event="job_snapshot"} | json | health = "FAIL" | unit =~ "borgbackup-.*" [10m]))'';
    })
    {
      id = 4;
